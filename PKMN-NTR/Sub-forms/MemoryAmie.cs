    ...
    public partial class MemoryAmie : Form
    {
        ...
        public MemoryAmie() // Keeping the form reference as a lot of control elements are required to operate.
        {
            ...
            foreach (ComboBox comboBox in cba)
            {
                ...
                MainForm.setCountrySubRegion(comboBox, "countries");
            }
            getLangStrings();
            loadFields();
        }
        ...
        private readonly PKM pkm = Program.gCmdWindow.pkm;
        ...
        private void getMemoryArguments(string ARG, ComboBox sender)
        {
            ...
            switch (ARG)
            {
                case "NONE":
                    ...
                    break;
                case "PKM":
                    argvals = Util.getCBList(GameInfo.Strings.specieslist.Take(Program.gCmdWindow.SAV.MaxSpeciesID + 1).ToArray(), null);
                    vs = vartypes[0];
                    break;
                case "GENLOC":
        ...
        private void changeCountryIndex(object sender, EventArgs e)
        {
            ...
            {
                MainForm.setCountrySubRegion(mta[index], "sr_" + WinFormsUtil.getIndex(sender as ComboBox).ToString("000"));
