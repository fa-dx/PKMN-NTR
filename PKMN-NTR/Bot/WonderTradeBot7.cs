...
namespace ntrbase.Bot
{
    class WonderTradeBot7
    {
        ...
        public WonderTradeBot7(int inputBox, int inputSlot, int Amount, bool FCaftertrade, int Sourcepkm, bool endlessrun, bool inputdumpafter, int inputafteraction)
        {
            botresult = 0;
            botstop = false;
            ...
            currentbox = inputBox - 1;
            currentslot = inputSlot - 1;
            startbox = inputBox - 1;
            startslot = inputSlot - 1;
            quantity = Amount;
            starttrades = Amount;
            source = Sourcepkm;
            endless = endlessrun;
                    ...
                    switch (botstate)
                    {
                        ...
                        case (int)botstates.backup:
                            ...
                            if (await waitTaskbool)
                            {
                                ...
                                botstate = (int)botstates.checkmode;
                            }
                            else
                            {
                                attempts++;
                        ...
                        case (int)botstates.readfolder:
                            ...
                            if (pkfiles.Length > 0)
                            {
                                ...
                                foreach (string pkf in pkfiles)
                                {
                                    ...
                                    {
                                        validator.Data = temp;
                                        if (validator.Species > 0 && validator.Species <= 802)
                                        { // Valid looking file
                                            pklist.Add(temp);
                                        }
                                        else
                        ...
                        case (int)botstates.delete1:
                            ...
                            int todelete = 0;
                            if (startbox * 30 + startslot + starttrades > 960)
                            { // Check if it overflows position
                                quantity = startbox * 30 + startslot + starttrades - 960;
